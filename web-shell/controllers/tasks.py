...
def tasks():
    ...
    if method == "POST":
        try:
            ...
            assert(type(form["object_name"]) == str)
            assert(type(form["task_name"]) == str)
            r = RPCRemote(CORE_APIS_RPC_ADDR, CORE_APIS_RPC_PORT)
            result = r.pc("add_task")(form["task_name"], form["object_name"], form["project_name"])
            assert(type(result) == list)
            assert(len(result) == 1)
            assert(type(result[0] == list))
            assert(len(result[0]) == 2)
            if result[0][0] == 0:
                return flask.redirect(flask.url_for("object", project_name=form["project_name"], object_name=form["object_name"]))
            else:
